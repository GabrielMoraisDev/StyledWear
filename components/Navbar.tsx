...
export default function Navbar({ page, ProductCount }: NavProps) {
  ...
  useEffect(() => {
    ...
    return (
        ...
        <div className={`fixed bottom-5 bg-slate-800 ${classc} rounded-md py-4 px-7 duration-300 text-white`}>Produto adicionado ao carrinho!</div>
            ...
            <div className={`inline sm:hidden absolute bg1 w-full ${menu ? 'h-1 pb-72' : 'h-0'} left-0 top-11 overflow-hidden text-white duration-300 mt-4`}>
                ...
                <Link href='/shirts'><p className={`${page==='shirt'?'opacity-70':''} ml-[6vw] mt-4`}>CAMISAS</p></Link>
                <Link href='/pants'><p className={`${page==='pants'?'opacity-70':''} mt-4 ml-[6vw]`}>CALÇAS</p></Link>
                <Link href='/watches'><p className={`${page==='watches'?'opacity-70':''} mt-4 ml-[6vw]`}>RELÓGIOS</p></Link>
                <Link href='/login'><p className={`${page==='login'?'opacity-70':''} mt-4 ml-[6vw]`}>ENTRAR</p></Link>
                <Link href='/cart'><p className={`${page==='cart'?'opacity-70':''} mt-4 ml-[6vw]`}>CARRINHO DE COMPRAS</p></Link>
            </div>

                <Link href='/' className={`${page==='home'?'opacity-65':'hover:opacity-65'} hidden sm:block sm:ml-3 sm:text-sm md:text-sm md:ml-10 xl:text-md xl:ml-20 text-white text-md cursor-pointer duration-300`}>INÍCIO</Link>
                <Link href='/shirts' className={`${page==='shirts'?'opacity-65':'hover:opacity-65'} hidden sm:block sm:ml-10 sm:text-sm md:text-sm md:ml-14 xl:text-md xl:ml-20 text-white text-md cursor-pointer duration-300`}>CAMISAS</Link>
                <Link href='/pants' className={`${page==='pants'?'opacity-65':'hover:opacity-65'} hidden sm:block sm:ml-3 sm:text-sm md:text-sm md:ml-10 xl:text-md xl:ml-20 text-white text-md cursor-pointer duration-300`}>CALÇAS</Link>
                <Link href='/watches' className={`${page==='watches'?'opacity-65':'hover:opacity-65'} hidden sm:block sm:ml-3 sm:text-sm md:text-sm md:ml-10 xl:text-md xl:ml-20 text-white text-md cursor-pointer duration-300`}>RELÓGIOS</Link>
                ...
                <div className="hidden sm:flex ml-auto sm:mr-10 sm:w-[40%] md:mr-12 lg:mr-14 xl:mr-20 w-[30%] h-12 mt-2">
                    ...
                    <Link href='/cart' className="relative bg-transparent border border-white text-white hover:bg-white hover:text-slate-800 rounded-full w-10 h-10 ml-3 flex justify-center place-items-center duration-300 cursor-pointer">
